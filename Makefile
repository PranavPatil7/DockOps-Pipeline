...
# Load project name from vault (fallback to default)
PROJECT_NAME := $(shell ansible-vault view $(VAULT_FILE) 2>/dev/null | grep vault_project_name | cut -d'"' -f2 2>/dev/null || echo "devops-stack")
...
setup: install ## Complete setup including SSH key verification
	...
	@if [ ! -f ~/.ssh/id_rsa ] && [ ! -f ~/.ssh/id_ed25519 ]; then \
		...
		echo "Please ensure your SSH private key is in ~/.ssh/"; \
		exit 1; \
	fi
